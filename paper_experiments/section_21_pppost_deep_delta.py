#!/usr/bin/env python3
"""Delta run: append deep EvLogit PPtheta-Post variants to old arch CSVs.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--target-stage", choices=sorted(TARGETS), default=os.environ.get("DEEP_TARGET_STAGE"))
    p.add_argument("--target-dataset", default=os.environ.get("DATASET"))
    p.add_argument(
        "--append-root",
        default=os.environ.get(
            "PPPOST_DEEP_APPEND_ROOT",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not known.target_stage:
        parser.error("--target-stage or DEEP_TARGET_STAGE is required")
    ...
    for grid_args in _target_grid(spec):
        ...
        print(
            ...
            f"append={append_csv.name} variants={DEEP_VARIANTS} "
