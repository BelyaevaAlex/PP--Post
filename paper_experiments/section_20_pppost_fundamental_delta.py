#!/usr/bin/env python3
"""Delta run: append fundamental PPtheta-Post variants to old arch CSVs.

This script intentionally runs only the new variants introduced after the
``mortality_pppost_arch_v1`` batch:
...
It appends their rows to each previous section CSV/JSONL without rerunning older
baselines or earlier PPtheta-Post variants.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--target-stage", choices=sorted(TARGETS), default=os.environ.get("FUNDAMENTAL_TARGET_STAGE"))
    p.add_argument("--target-dataset", default=os.environ.get("DATASET"))
    p.add_argument(
        "--append-root",
        default=os.environ.get(
            "PPPOST_FUNDAMENTAL_APPEND_ROOT",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not known.target_stage:
        parser.error("--target-stage or FUNDAMENTAL_TARGET_STAGE is required")
    ...
    for grid_args in _target_grid(spec):
        ...
        print(
            "[pppost-fundamental-delta] "
            f"dataset={known.target_dataset} target_stage={known.target_stage} "
            f"append={append_csv.name} variants={FUNDAMENTAL_VARIANTS} "
