#!/usr/bin/env python3
"""Delta run: append Source-Calibrated PPtheta-Post rows to old arch CSVs.

This script intentionally runs only ``pp_theta_post_source_calibrated``.  It is
used after the ``mortality_pppost_arch_v1`` batch so the new model can be added
to each previous section CSV/JSONL without rerunning older baselines or PPtheta
variants.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--target-stage", choices=sorted(TARGETS), default=os.environ.get("SOURCE_CAL_TARGET_STAGE"))
    p.add_argument("--target-dataset", default=os.environ.get("DATASET"))
    p.add_argument(
        "--append-root",
        default=os.environ.get(
            "SOURCE_CAL_APPEND_ROOT",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not known.target_stage:
        parser.error("--target-stage or SOURCE_CAL_TARGET_STAGE is required")
    ...
    for grid_args in _target_grid(spec):
        args = (
            ...
            + [
                ...
                "--baselines", "none",
                "--variants", "pp_theta_post_source_calibrated",
        ...
        print(
            "[source-calibrated-delta] "
            f"dataset={known.target_dataset} target_stage={known.target_stage} "
            f"append={append_csv.name} grid={' '.join(grid_args) or 'default'}"
        )
