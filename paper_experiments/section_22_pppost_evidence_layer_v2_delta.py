#!/usr/bin/env python3
"""Delta run: append PPtheta-Post Evidence Layer v2 rows to old arch CSVs.

This script intentionally runs only ``pp_theta_post_evidence_layer_v2`` after
...
EVIDENCE_V2_VARIANTS = "pp_theta_post_evidence_layer_v2"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--target-stage", choices=sorted(TARGETS), default=os.environ.get("EVIDENCE_V2_TARGET_STAGE"))
    p.add_argument("--target-dataset", default=os.environ.get("DATASET"))
    p.add_argument(
        "--append-root",
        default=os.environ.get(
            "PPPOST_EVIDENCE_V2_APPEND_ROOT",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not known.target_stage:
        parser.error("--target-stage or EVIDENCE_V2_TARGET_STAGE is required")
    ...
    for grid_args in _target_grid(spec):
        ...
        print(
            "[pppost-evidence-v2-delta] "
            f"dataset={known.target_dataset} target_stage={known.target_stage} "
            f"append={append_csv.name} variant={EVIDENCE_V2_VARIANTS} "
