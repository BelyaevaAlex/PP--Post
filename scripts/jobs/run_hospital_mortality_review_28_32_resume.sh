#!/usr/bin/env bash
# Resume reviewer-defense run after Sections 25-27 have produced artifacts.
# Continues with Sections 28, 29, 30, 31, 32 and a final Section 29 refresh.
set -euo pipefail
...
RUN_ID="${RUN_ID:-hospital_mortality_48h_sections_25_32_v2}"
...
LOG="$LOG_ROOT/resume_$(date +%Y%m%d_%H%M%S).log"
...
run_step_if_missing() {
  ...
  shift 2
  if [ -s "$expected" ]; then
    log_status "$section" skipped 0 "present:$expected"
    echo "[$(date -Iseconds)] SKIP $section expected=$expected"
    return 0
  fi
  run_step "$section" "$@"
}

CLIN_CSV="${CLIN_CSV:-$(ls -t "$RUN_ROOT"/26_clinical_metrics/compare_datasets_*.csv | head -n 1)}"
if [ -z "$CLIN_CSV" ] || [ ! -s "$CLIN_CSV" ]; then
  echo "Missing clinical CSV under $RUN_ROOT/26_clinical_metrics" >&2
  exit 2
fi
...
run_step_if_missing section_30_posterior_parity_complexity "$RUN_ROOT/30_posterior_parity_complexity/POSTERIOR_PARITY_COMPLEXITY.md" \
  "$PY" paper_experiments/section_30_posterior_parity_complexity.py \
  --output-dir "$RUN_ROOT/30_posterior_parity_complexity"

run_step_if_missing section_31_audit_faithfulness "$RUN_ROOT/31_audit_faithfulness/AUDIT_FAITHFULNESS_SUMMARY.md" \
  "$PY" paper_experiments/section_31_audit_faithfulness.py \
  --case-root "$CASE_ROOT" \
  --output-dir "$RUN_ROOT/31_audit_faithfulness"

run_step_if_missing section_32_explanation_baselines "$RUN_ROOT/32_explanation_baselines/EXPLANATION_BASELINE_PROTOCOL.md" \
...
echo "STATUS=$STATUS"
echo "LOG=$LOG"
echo "[done] ${RUN_ID}_resume $(date -Iseconds)"
